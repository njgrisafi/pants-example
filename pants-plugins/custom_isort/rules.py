...
from custom_isort.skip_field import SkipIsortField
...
def generate_argv(source_files: SourceFiles, isort: Isort, *, is_isort5: bool, check_only: bool) -> Tuple[str, ...]:
    args = [*isort.args]
    if check_only:
...
async def setup_isort(setup_request: SetupRequest, isort: Isort) -> Setup:
    ...
    config_files = await Get(ConfigFiles, ConfigFilesRequest, isort.config_request(source_files_snapshot.dirs))
    ...
    if isort.config:
        ...
        is_isort5 = any(dist_info.project_name == "isort" and dist_info.version.major >= 5 for dist_info in isort_info)

    input_digest = await Get(Digest, MergeDigests((source_files_snapshot.digest, config_files.snapshot.digest)))
