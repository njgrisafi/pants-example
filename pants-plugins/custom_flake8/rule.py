...
class Flake8FieldSet(FieldSet):
    ...
    def opt_out(cls, tgt: Target) -> bool:
        opt_out = tgt.get(SkipFlake8Field).value
        logger.debug(f"{str(tgt.address)} {opt_out}")
        return opt_out
...
def generate_argv(source_files: SourceFiles, flake8: Flake8, *, report_file_name: Optional[str]) -> Tuple[str, ...]:
    args = []
    if flake8.config:
...
async def flake8_lint_partition(
    ...
    source_files_get = Get(SourceFiles, SourceFilesRequest(field_set.sources for field_set in partition.field_sets))
    flake8_pex, config_files, source_files = await MultiGet(flake8_pex_get, config_files_get, source_files_get)

    input_digest = await Get(Digest, MergeDigests((source_files.snapshot.digest, config_files.snapshot.digest)))
...
async def flake8_lint(request: Flake8Request, flake8: Flake8, python_setup: PythonSetup) -> LintResults:
    if flake8.skip:
