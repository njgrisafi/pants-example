...
class ImportFixerHandler:
    ...
    def get_transitive_python_file_import_recommendations(
        ...
        recs = self.get_wildcard_import_recommendation(
            ...
            py_wildcard_import=wildcard_py_import,
        )
    ...
    def get_wildcard_import_recommendation(
        ...
        py_wildcard_import: PythonImport,
    ) -> Tuple[PythonImport, ...]:
        visited = []
        ...
        while stack:
            ...
            try:
                transitive_py_file_info = self.py_package_helper.py_file_info_by_module[py_import.modules_str]
                import_recommendations.extend(
                    self.get_transitive_import_recommendations(
                        source_py_file_info=source_py_file_info,
                ...
                import_recommendations.extend(
                    self.get_submodule_import_recommendations_for_python_file(
                        source_py_file_info=source_py_file_info,
                        module_py_import=py_import,
                    )
                )

                # Queue all submodules for transitive checks
                stack.extend(self.get_submodule_transitive_imports(module_py_import=py_import))
                continue
    ...
    def get_transitive_import_recommendations(
        self, source_py_file_info: PythonFileInfo, transitive_py_file_info: PythonFileInfo
    ) -> Tuple[PythonImport, ...]:
        import_recommendations = []
        ...
        names = self.py_package_helper.get_names_used_from_transitive_python_file(
            source_py_file=source_py_file_info, transitive_py_file=transitive_py_file_info
        )
        if names:
            import_recommendations.append(
                PythonImport(
                    modules=tuple(transitive_py_file_info.module_key.split(".")), level=0, names=names, aliases=()
                )
            )
    ...
    def get_submodule_import_recommendations_for_python_file(
        self, source_py_file_info: PythonFileInfo, module_py_import: PythonImport
    ) -> Tuple[PythonImport, ...]:
        module_directory_python_imports = []
        for module_key, python_file_info in self.py_package_helper.py_file_info_by_module.items():
            symbol = python_file_info.module_key.split(".")[-1]
            if module_py_import.modules_str in module_key and utils.has_symbol_usage(
                symbol=symbol, file_content=source_py_file_info.file_content_str
            ):
                module_directory_python_imports.append(
                    PythonImport(modules=module_py_import.modules, level=0, names=(symbol,), aliases=())
                )
        return module_directory_python_imports

    def get_submodule_transitive_imports(self, module_py_import: PythonImport) -> List[PythonImport]:
        submodule_python_imports = []
        for module_key in self.py_package_helper.py_file_info_by_module:
            if module_py_import.modules_str in module_key:
    ...
    def get_file_duplicate_import_recommendations(
        ...
        for duplicate_import in duplicate_imports:
            if f"{duplicate_import.modules_str}.{duplicate_name}" in self.py_package_helper.py_file_info_by_module:
                direct_name_definitions.append(duplicate_import)
            elif duplicate_import.modules_str in self.py_package_helper.py_file_info_by_module:
                file_info = self.py_package_helper.py_file_info_by_module[duplicate_import.modules_str]
    ...
    def get_missing_import_recommendation(self, missing_name: str) -> PythonImportRecommendation:
        ...
        for module_str, py_file_info in self.py_package_helper.py_file_info_by_module.items():
