...
from .subsystem import Autoflake
...
def generate_args(*, source_files: SourceFiles, autoflake: Autoflake, check_only: bool) -> Tuple[str, ...]:
...
async def setup_autoflake(setup_request: SetupRequest, autoflake: Autoflake) -> Setup:
    ...
    process = await Get(
        ...
        VenvPexProcess(
            autoflake_pex,
            argv=generate_args(source_files=source_files, autoflake=autoflake, check_only=setup_request.check_only),
